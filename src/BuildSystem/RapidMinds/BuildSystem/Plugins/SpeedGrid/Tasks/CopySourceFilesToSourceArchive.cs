...
namespace Plugin.SpeedGrid.Tasks
{
    public class CopySourceFilesToSourceArchive 
    {
        ...
        public void Execute()
        {
            ...
            // /F "%teamcity.build.checkoutDir%\SpeedGrid\Properties\AssemblyInfo.cs" "%env.BUILD_NUMBER%" /APPEND
            ...
            destination = Path.Combine(destination, "SpeedGrid");   // We need to keep the "perforce path" intact so the ModifyInstallShield tool will work
