...
parser.add_argument("--trace", required=False, help="Adding trace to some actions.")
...
if os.path.isfile(orig_file_path):
    if my_args.trace is not None:
        print("File already cached locally. Skipping download.")
else:

    print("Trying to download file from '{0}' into '{1}'.".format(my_args.fileurl, orig_file_path))
