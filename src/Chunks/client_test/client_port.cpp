...
port_number) try {
	...
	the_monitor_interface->register_client_process(pid, the_owner_id); /// TODO: Timeout.
	...
	if (the_owner_id.get_owner_id() == owner_id::none) {
		...
		return SCERRCLIENTACQUIREOWNERID;
	}
...
void release_channel(void* port, uint32_t the_channel_index, uint32_t the_scheduler_number)
{
	...
	client_port* the_port;
	the_port = (client_port*)port;
	the_port->release_channel(the_channel_index, the_scheduler_number);
}
