...
monitor_interface_name, pid_type pid, owner_id oid /*, std::size_t id*/)
: shared_interface(segment_name, monitor_interface_name, pid, oid) /*, id_(id)*/
{}
...
inline chunk_index client_port::wait_for_response(uint32_t the_channel_index) {
	...
	channel(the_channel_index).out.pop_back(&the_chunk_index);
...
inline chunk_index client_port::acquire_chunk() {
	...
	while (!acquire_chunk_index(&index)) {
...
the_scheduler_number) {
	channel_number the_channel_number;
	
	/// TODO: Handle errors.
	
	if (!acquire_channel_number(&the_channel_number,
	the_scheduler_number)) {
		//...
	}
