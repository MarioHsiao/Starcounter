...
#ifndef STARCOUNTER_CORE_OVERFLOW_BUFFER_HPP
#define STARCOUNTER_CORE_OVERFLOW_BUFFER_HPP
...
// The overflow_buffer relies on Boost Threads and Boost Bind libraries and
// Boost call_traits utility.

// The push_front() method is called in order to insert a new item into the
// buffer. If there is a space in the buffer available, the execution continues
// and the method inserts the item at the end of the overflow_buffer. Then it
// increments the number of unread items.

// The pop_back() method is called in order to read the next item from the
// buffer. If there is at least one unread item, the method decrements the
// number of unread items and reads the next item from the overflow_buffer.

// The pop_back() method does not remove the item but the item is left in the
// overflow_buffer which then replaces it with a new one (inserted by a
// producer) when the overflow_buffer is full. This technique is more effective
// than removing the item explicitly by calling the pop_back() method of the
// overflow_buffer. This claim is based on the assumption that an assignment
// (replacement) of a new item into an old one is more effective than a
// destruction (removal) of an old item and a consequent inplace construction
// (insertion) of a new item.

/// class overflow_buffer
/**
 * @param T The type of the elements stored in the overflow_buffer.
 *
 ...
 *      if the overflow_buffer will be compared with another container.
 *
...
public:
	...
	// The type of the underlying container used in the overflow_buffer.
	typedef boost::circular_buffer<T, Alloc> container_type;

	// The type of elements stored in the overflow_buffer.
	...
	/// Create an empty overflow_buffer with the specified capacity.
	/**
	 * @param buffer_capacity The maximum number of elements which can be stored
	 *      in the overflow_buffer.
	 *
	 ...
	 *      Constant.
	 */
	explicit overflow_buffer(size_type buffer_capacity, const allocator_type&
	alloc = allocator_type());
	
	// Size and capacity

	/// Get the number of elements currently stored in the overflow_buffer.
	/**
	 * @return The number of elements stored in the overflow_buffer.
	 *
	 * @throws Nothing.
	 *
	 ...
	 *
	 * @par Complexity
	 *      Constant (in the size of the overflow_buffer).
	 */
	size_type size() const;

	/// Get the capacity of the overflow_buffer.
	/**
	...
	/// Is the overflow_buffer empty?
	/**
	 * @return true if there are no elements stored in the overflow_buffer;
	 *      false otherwise.
	 *
	 * @throws Nothing.
	 *
	...
	/// Is the overflow_buffer full?
	/**
	 * @return true if the number of elements stored in the overflow_buffer
	 *      equals the capacity of the overflow_buffer; false otherwise.
	 *
	 * @throws Nothing.
	 *
	 * @par Exception Safety
...
#include "impl/overflow_buffer.hpp"

#endif // STARCOUNTER_CORE_OVERFLOW_BUFFER_HPP
