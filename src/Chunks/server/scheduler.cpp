...
check_next_channel:
        ...
        mask_word_counter < channel_masks_; ++mask_word_counter) {
            ...
            mask; mask &= mask -1) {
                channel_number this_channel = static_cast<channel_number>(bit_scan_forward(mask)); // JLI warning fix
                this_channel += mask_word_counter << 6;
...
long server_port::has_task() {
    ...
    for (channel_number n = 0; n < channel_masks_; ++n) {
        ...
        ->get_channel_mask_word(n); mask; mask &= mask -1) {
            uint32_t ch = static_cast<uint32_t>(bit_scan_forward(mask)); // JLI warning fix
            ch += n << 6;
...
unsigned long server_port::acquire_linked_chunk_indexes(unsigned long start_chunk_index, unsigned long needed_size)
{
    ...
    uint32_t num_needed_chunks = static_cast<uint32_t>(div_value.quot); // JLI warning fix
    if (div_value.rem != 0)
        num_needed_chunks++;
...
unsigned long server_port::release_linked_chunks(chunk_index start_chunk_index)
{
    ...
    int32_t num_to_return = static_cast<int32_t>(this_scheduler_interface_->chunk_pool().size() - a_bunch_of_chunks); // JLI warning fix
    if (num_to_return > 0)
    {
