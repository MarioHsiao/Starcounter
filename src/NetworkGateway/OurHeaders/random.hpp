#pragma once
#ifndef STARCOUNTER_NETWORK_RANDOM_HPP
#define STARCOUNTER_NETWORK_RANDOM_HPP
...
public:
	 ...
	 */
	uint16_t uint16() {
		return static_cast<uint16_t>(uint64());
	}
...
// Implementation of high-quality random hash of an integer into several numeric
// types.
class random_hash {
public:
	 ...
	 *      Constant.
	 */
	uint32_t uint32(uint64_t key) {
		return static_cast<uint32_t>(uint64(key));
	}
	 ...
	 *      Constant.
	 */
	uint32_t uint16(uint64_t key) {
		return static_cast<uint16_t>(uint64(key));
	}
	 ...
	 *      Constant.
	 */
	uint8_t uint8(uint64_t key) {
		return static_cast<uint8_t>(uint64(key));
	}
	...
	double doub(uint64_t key) {
		return 5.42101086242752217E-20 * uint64(key);
	}
};
...
#endif // STARCOUNTER_NETWORK_RANDOM_HPP
