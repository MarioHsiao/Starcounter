        ...
        internal string DebugMatchFragment {
            get {
                ...
                if (Parent.Parent != null)
                    start = Parent.Parent.MatchCharInTemplateAbsolute + 1;
                //              if (HandlerIndex == -1) {
                string str = "";
                if (HandlerIndex == -1) {
                    str = GetAnyCandidateHandler().UnpreparedVerbAndUri;
                    //                else
                    //                    str = DebugHandlers[HandlerIndex].verbAndUri;
                    var stop = Parent.MatchCharInTemplateAbsolute;
                    var cnt = stop - start;
        ...
        private string BuildPrettyPrintDebugString(bool fullDebug, StringBuilder sb, int indentation) {
            ...
            if (HandlerIndex != -1 && Candidates.Count == 0) {
                if (par)
                    sb.Append(", ");
                sb.Append("Handler:\"");
                sb.Append(AllHandlers[HandlerIndex].UnpreparedVerbAndUri);
                sb.Append('"');
                par = true;
            }
            if (IsParseTypeNode) {
            ...
            if (Candidates.Count == 0) {
                if (HandlerIndex == -1)
                    return " // ERROR";
                return " // " + AllHandlers[HandlerIndex].UnpreparedVerbAndUri;
            }
            return comment;
        }
