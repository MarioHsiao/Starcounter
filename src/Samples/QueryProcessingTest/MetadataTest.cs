...
namespace QueryProcessingTest {
    public static class MetadataTest {
        ...
        public static void TestRuntimeColumnMetadata() {
            ...
            Trace.Assert(c.MaterializedColumn.Nullable == c.Nullable);
            Trace.Assert(!c.Unique);
#if false
            int nrColumns = 0;
        ...
        public static void ClrMetadatTest() {
            ...
            c = Db.SQL<MappedProperty>("select c from starcounter.metadata.MappedProperty c where name = ? and c.table is ClrClass order by c desc", 
                "WriteLoss").First;
            Trace.Assert(c != null);
