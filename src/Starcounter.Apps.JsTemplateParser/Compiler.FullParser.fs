// Starcounter Internal Template Parser (accepting full Ecmascript 3.0 source code)
...
module internal FullParser =
  ...
  type State = {
    ...
    ScopeData : Parser.ScopeData ref
    FunctionData : Parser.ScopeData ref
  } 
//    #if DEBUG
    with
    member x.TokenName = 
  ...
  let create position prettyPrint = {
    ...
    ScopeData = ref Unchecked.defaultof<Parser.ScopeData>
    FunctionData = ref Unchecked.defaultof<Parser.ScopeData>
  }
  ...
  let toOctal (s:string, di:DebugInfo) = (Convert.ToInt32(s, 8) |> double, di)
...
//    let mutable bi = Unchecked.defaultof<Numerics.BigInteger>
  ...
  let simplefWithDebug symbol f (p:P) =
    p |> nud symbol (fun t p -> p |> consume; (t,DebugInfo(p)) |> f)
  ...
  let try' _ p =
    ...
    | S.Catch -> 
      ...
      let scopeData = Parser.ScopeData.New id catchScope (Some parentData)
  ...
  let call' _ functionAst p = 
    ...
    | Ast.Identifier ("eval",debugInfo) -> 
      ...
      Ast.Tree.Eval(argAsts |> FSharp.List.headOr (lazy Ast.Tree.String("",DebugInfo(0,0,""))))
  ...
  let function' isDefinition _ p =
    ...
    let buildScope (p:P) =
      ...
      let scopeData = 
        ...
        Parser.ScopeData.New id scope parent
  ...
  let directive _ (p:P) =
    ...
    | _ -> Error.CompileError.Raise( "Jocke error" ) // JOCKE
  ...
  let internal parserDefinition =
    ...
    *)
    ...
    |> simple S.Null (Ast.Tree.Null (DebugInfo(0,0,"")))
    |> simple S.True (Ast.Tree.Boolean (true,DebugInfo(0,0,"")))
    |> simple S.False (Ast.Tree.Boolean (false,DebugInfo(0,0,"")))
    |> simple S.This Ast.Tree.This
    |> simplefWithDebug S.String (valueWithDebug >> Ast.Tree.String)
  ...
  let parse (source:Src) = // JOCKE
    ...
    let scopeData =
      Parser.ScopeData.New 0 (Ast.ScopeOption.Function scope) None // JOCKE TODO! Reintroduce uint64 (0UL) when I figure out how to
    ...
    Ast.Tree.Function(None, scope, ast, DebugInfo(0,0,"") ), scopeData
    
//  let parseString env string = JOCKE 
//    env |> parse string
