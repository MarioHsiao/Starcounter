...
module Error =
  ...
  type CompileError(msg, pos, source, path) = 
    ...
    static member Raise(msg, pos, source:string, path, overhead ) = // JOCKE
      let line = fst pos
      let col, cleanSource = 
          if ( line <> 1 ) then
             ( snd pos, source )
          else
             ( snd pos - overhead, source.Substring(overhead) )
      let fixedSource = cleanSource |> sprintError (line,col)
      raise(CompileError(msg, Some (line,col), Some fixedSource, Some path))
