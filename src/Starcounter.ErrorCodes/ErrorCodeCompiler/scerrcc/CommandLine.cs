...
static class CommandLine
{
        ...
        for (int i = 1; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                case "-ea":
                    ...
                    break;
                default:
                    Program.Die("Unknown switch: " + args[i]);
                    break;
            }
        }
    }
    ...
    private static void PrintUsageAndExit()
    {
        ...
        Program.Die("Filenames can be - which means stdin/stdout");
    }
    ...
    private static void GetCSharpStream(string[] args, DateTime inputLastModified, ref TextWriter csfile, ref int i)
    {
        if (++i == args.Length)
        {
            Program.Die("-cs switch requires a filename argument");
        }
        else if (csfile != null)
        {
            Program.Die("C# file already opened");
        }
        else if (args[i] == "-")
        {
    ...
    private static void GetCSharpStream2(string[] args, DateTime inputLastModified, ref TextWriter csfile, ref int i)
    {
        if (++i == args.Length)
        {
            Program.Die("-cs2 switch requires a filename argument");
        }
        else if (csfile != null)
        {
            Program.Die("C# file already opened");
    ...
    private static void GetMcStream(string[] args, DateTime inputLastModified, ref TextWriter mcfile, ref int i)
    {
        if (++i == args.Length)
        {
            Program.Die("-mc switch requires a filename argument");
        }
        else if (mcfile != null)
        {
            Program.Die("MC file already opened");
        }
        else if (args[i] == "-")
        {
    ...
    private static void GetEAContentStream(string[] args, ref TextWriter contentFile, ref int i)
    {
        if (++i == args.Length)
        {
            Program.Die("-ea switch requires a filename argument");
        }
        else if (contentFile != null)
        {
            Program.Die("Excepction assistant content file already opened");
        }
        else if (args[i] == "-")
        {
    ...
    private static StreamWriter OpenWritableIfYounger(string fn, DateTime inputLastModified)
    {
        ...
        {
            Program.Die("File " + fn + " is read-only. Did you remember to check it out?");
        }
