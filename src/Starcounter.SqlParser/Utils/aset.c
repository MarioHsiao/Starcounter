	...
	if (minContextSize > ALLOC_BLOCKHDRSZ + ALLOC_CHUNKHDRSZ)
	{
		...
		if (block == NULL)
		{
			...
			errprint("\nERROR OUT_OF_MEMORY: out of memory\nFailed while creating memory context \"%s\".",
				name);
			ThrowException(ScErrMessageStr("Out of memory. Failed while creating memory context \"%s\".",
				name));
		}
		block->aset = context;
...
AllocSetAlloc(MemoryContext context, Size size)
{
	...
	if (size > set->allocChunkLimit)
	{
		...
		{
			MemoryContextStats(TopMemoryContext);
			errprint("\nERROR OUT_OF_MEMORY: out of memory\nFailed on request of size %lu.",
				(unsigned long) size);
			ThrowException(ScErrMessageLU("Out of memory. Failed on request of size %lu.",
				(unsigned long) size));
		}
		block->aset = set;
	...
	if (block == NULL)
	{
		...
		if (block == NULL)
		{
			...
			errprint("\nERROR OUT_OF_MEMORY: out of memory\nFailed on request of size %lu.",
				(unsigned long) size);
			ThrowException(ScErrMessageLU("Out of memory. Failed on request of size %lu.", (unsigned long) size));
		}
...
AllocSetFree(MemoryContext context, void *pointer)
{
	...
	if (chunk->requested_size < chunk->size)
		...
		{
			errprint("\nWARNING: detected write past chunk end in %s %p",
	...
	if (chunk->size > set->allocChunkLimit)
	{
		...
		if (block == NULL)
		{
			errprint("\nERROR: could not find block containing chunk %p", chunk);
			ThrowException(ScErrMessagePointer("could not find block containing chunk %p", chunk));
		}
...
AllocSetRealloc(MemoryContext context, void *pointer, Size size)
{
	...
	if (oldsize > set->allocChunkLimit)
	{
		...
		if (block == NULL)
		{
			...
			errprint("\nERROR OUT_OF_MEMORY: out of memory\nFailed on request of size %lu.",
				(unsigned long) size);
			ThrowException(ScErrMessageLU("Failed on request of size %lu.",(unsigned long) size));
		}
		block->freeptr = block->endptr = ((char *) block) + blksize;
...
AllocSetCheck(MemoryContext context)
{
	...
	for (block = set->blocks; block != NULL; block = block->next)
	{
		...
		if (!blk_used)
		{
			if (set->keeper != block)
			{
				errprint("\nWARNING: problem in alloc set %s: empty block %p",
					name, block);
				ThrowException(ScErrMessageStrPointer("problem in alloc set %s: empty block %p", name, block));
			}
		}
		...
		while (bpoz < block->freeptr)
		{
			...
			if (dsize > chsize)
			{
				errprint("\nWARNING: problem in alloc set %s: req size > alloc size for chunk %p in block %p",
					name, chunk, block);
			}
			if (chsize < (1 << ALLOC_MINBITS))
			{
				errprint("\nWARNING: problem in alloc set %s: bad size %lu for chunk %p in block %p",
			...
			{
				errprint("\nWARNING: problem in alloc set %s: bad single-chunk %p in block %p",
					name, chunk, block);
			}
			...
			{
				errprint("\nWARNING: problem in alloc set %s: bogus aset link in block %p, chunk %p",
					name, block, chunk);
			}
			...
			{
				errprint("\nWARNING: problem in alloc set %s: detected write past chunk end in block %p, chunk %p",
					name, block, chunk);
			}
		...
		{
			errprint("\nWARNING: problem in alloc set %s: found inconsistent memory block %p",
				name, block);
		}
	}
}
