...
enlargeStringInfo(StringInfo str, int needed)
{
	...
	{
		errprint("\nERROR: invalid string enlargement request size: %d", needed);
		ThrowException(ScErrMessageInt("Invalid string enlargement request size: %d", needed));
	}
	if (((Size) needed) >= (MaxAllocSize - (Size) str->len))
	{
		errprint("\nERROR PROGRAM_LIMIT_EXCEEDED: out of memory\nCannot enlarge string buffer containing %d bytes by %d more bytes.",
			str->len, needed);
		ThrowException(ScErrMessageInt("Invalid string enlargement request size: %d", needed));
	}
