...
module public Lexer =
  ...
  module Symbol =
    ...
    let [<Literal>] Integer = 86        // cho
    let [<Literal>] Double = 87         // cho
    let [<Literal>] Decimal = 88        // cho
  ...
  let private numericLiteral (c:char) (s:Input.T) =
    ...
    if s |> continue' then
      ...
      while s |> continue' && (c |> isDecimal || c = 'e' || c = 'E' || (passedExponent && (c = '-' || c = '+')) || (c = '.' && not passedDot)) do
        if not passedExponent then passedExponent <- c = 'e' || c = 'E'
        if not passedDot then passedDot <- c = '.'
        c |> buffer s
        s |> advance
        c <- s |> current

    if (passedExponent) then // We treat values with exponent as double.
        s |> outputBuffer Symbol.Double
    elif passedDot then // Default type for values with decimals is decimal
        s |> outputBuffer Symbol.Decimal
    else
        s |> outputBuffer Symbol.Integer
