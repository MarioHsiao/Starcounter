// ***********************************************************************
// <copyright file="DateTimeProperty.cs" company="Starcounter AB">
...
/// </summary>
internal class DateTimeProperty : Property, IDateTimePathItem
{
    /// <summary>
    /// Constructor.
    /// </summary>
    ...
    internal DateTimeProperty(Int32 extNum, ITypeBinding typeBind, IPropertyBinding propBind)
    {
        if (typeBind == null)
        {
    ...
    public Nullable<DateTime> EvaluateToDateTime(IObjectView obj)
    {
        ...
        if (obj is Row)
        {
            ...
            return partObj.GetDateTime(propIndex);
        }
        // Control that the type (obj.TypeBinding) of the input object
        // is equal to or a subtype (TypeBinding.SubTypeOf(TypeBinding)) of the type (typeBinding) to which this property belongs
        // is not implemented due to that interfaces cannot be handled and computational cost.
        return obj.GetDateTime(propIndex);
    }
    ...
    public IDateTimeExpression Instantiate(Row obj)
    {
        ...
        {
            return new DateTimeLiteral(EvaluateToDateTime(obj));
        }
    ...
    {
        stringBuilder.Append(tabs, "DateTimeProperty(");
    ...
    {
        stringGen.AppendLine(CodeGenStringGenerator.CODE_SECTION_TYPE.FUNCTIONS, "GetDatetimeProperty();");
    }
}
}
