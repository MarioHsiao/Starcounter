// ***********************************************************************
// <copyright file="UIntegerProperty.cs" company="Starcounter AB">
...
/// </summary>
internal class UIntegerProperty : Property, IUIntegerPathItem
{
    /// <summary>
    /// Constructor.
    /// </summary>
    ...
    internal UIntegerProperty(Int32 extNum, ITypeBinding typeBind, IPropertyBinding propBind)
    {
        if (typeBind == null)
        {
        ...
        if (propBind.TypeCode != DbTypeCode.Byte && propBind.TypeCode != DbTypeCode.UInt16 &&
    ...
    public Nullable<UInt64> EvaluateToUInteger(IObjectView obj)
    {
        ...
        if (obj is Row)
        {
            ...
            return partObj.GetUInt64(propIndex);
        }
        // Control that the type (obj.TypeBinding) of the input object
        // is equal to or a subtype (TypeBinding.SubTypeOf(TypeBinding)) of the type (typeBinding) to which this property belongs
        // is not implemented due to that interfaces cannot be handled and computational cost.
        return obj.GetUInt64(propIndex);
    }
    ...
    {
        stringBuilder.Append(tabs, "UIntegerProperty(");
    ...
    {
        stringGen.AppendLine(CodeGenStringGenerator.CODE_SECTION_TYPE.FUNCTIONS, "GetUIntegerProperty();");
    }
}
}
