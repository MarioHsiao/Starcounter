...
{
internal class BinaryDynamicRange : DynamicRange, IDynamicRange
{
    // Pre-cached range values.
    BinaryRangeValue lower, upper;
    List<BinaryRangePoint> rangePointList;

    internal BinaryDynamicRange()
    {
        rangePointList = new List<BinaryRangePoint>();
        logExpr = null;
        lower = new BinaryRangeValue();
        upper = new BinaryRangeValue();
    }

    private BinaryDynamicRange(List<BinaryRangePoint> rangePointList)
    {
    ...
    internal void AddRangePoint(BinaryRangePoint rangePoint)
    {
        switch (rangePoint.Operator)
        {
            // Replace "x = a" with "x >= a" and "x <= a".
            case ComparisonOperator.Equal:
                rangePointList.Add(new BinaryRangePoint(ComparisonOperator.GreaterThanOrEqual, rangePoint.Expression));
                rangePointList.Add(new BinaryRangePoint(ComparisonOperator.LessThanOrEqual, rangePoint.Expression));
                break;
            ...
            case ComparisonOperator.ISNOT:
                rangePointList.Add(new BinaryRangePoint(ComparisonOperator.GreaterThan, rangePoint.Expression));
                break;
            ...
            case ComparisonOperator.LessThanOrEqual:
                ...
                rangePointList.Add(new BinaryRangePoint(ComparisonOperator.GreaterThan, new BinaryLiteral(null)));
                break;
    ...
    public void CreateRangePointList(List<ILogicalExpression> conditionList, Int32 extentNumber, String strPath)
    {
        ...
        for (Int32 i = 0; i < conditionList.Count; i++)
        {
            ...
            if (!(rangePoint is BinaryRangePoint))
            {
                throw ErrorCode.ToException(Error.SCERRSQLINTERNALERROR, "Incorrect rangePoint.");
            }
            AddRangePoint(rangePoint as BinaryRangePoint);
            if (logExpr == null)
            {
        ...
        ref ComparisonOperator secondOp)
    {
        ...
        BinaryRangeValue rangeValue = null;
    ...
    {
        List<BinaryRangePoint> rangePointListClone = new List<BinaryRangePoint>();
        for (Int32 i = 0; i < rangePointList.Count; i++)
        {
            rangePointListClone.Add(rangePointList[i].Clone(varArray));
        }
        return new BinaryDynamicRange(rangePointListClone);
    }

    public void BuildString(MyStringBuilder stringBuilder, Int32 tabs)
    {
        stringBuilder.AppendLine(tabs, "BinaryDynamicRange(");
        for (Int32 i = 0; i < rangePointList.Count; i++)
        {
    ...
    {
        stringGen.AppendLine(CodeGenStringGenerator.CODE_SECTION_TYPE.FUNCTIONS, "Binary range: " + lower.GetValue + " - " + upper.GetValue);
    }
}
}
