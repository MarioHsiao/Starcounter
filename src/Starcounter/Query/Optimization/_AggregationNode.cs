...
internal class AggregationNode : IOptimizationNode
{
        ...
        Boolean topNode, ref Byte nodeId)
    {
        ...
        IExecutionEnumerator subEnumerator = subNode.CreateExecutionEnumerator<T>(null, null, null, false, ref nodeId);
        IQueryComparer comparer = sortSpec.CreateComparer();
        return new Aggregation<T>(nodeId++, rowTypeBind, extentNumber, subEnumerator, comparer, setFunctionList, havingCondition, 
            variableArr, query, fetchNumExpr, fetchOffsetExpr, fetchOffsetKeyExpr, topNode);
    }
