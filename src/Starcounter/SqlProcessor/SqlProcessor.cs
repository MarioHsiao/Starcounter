...
namespace Starcounter.SqlProcessor {
    public class SqlProcessor {
        ...
        [DllImport("scsqlprocessor.dll")]
        internal static unsafe extern uint scsql_process_modifyquery(long transaction_handle, [MarshalAs(UnmanagedType.LPWStr)]string query, 
            int* nrObjs);
        [DllImport("scsqlprocessor.dll")]
        ...
        internal static unsafe int ExecuteQuerySqlProcessor(String query) {
            int nrObjs = 0;
      ulong handle;
            sccoredb.star_context_get_current_transaction(
                                ThreadData.ContextHandle, out handle
                                );

      uint err = scsql_process_modifyquery((long)handle, query, &nrObjs);
            if (err == 0)
                return nrObjs;
            Exception ex = GetSqlException(err, query);
            Debug.Assert(err == (uint)ex.Data[ErrorCode.EC_TRANSPORT_KEY]);
