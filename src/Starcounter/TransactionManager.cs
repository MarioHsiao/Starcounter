...
namespace Starcounter.Internal {
    ...
    internal class TransactionManager : ITransactionManager {
        ...
        public TransactionHandle Create(bool readOnly, bool detectConflicts) {
            ...
            ec = sccoredb.star_create_transaction(flags, out handle, out verify);
            if (ec == 0) {
                try {
                    ...
                    sccoredb.star_free_transaction(handle, verify);
                    throw;
                }
            }
        ...
        private static TransactionHandle CreateAndSetCurrent(bool readOnly, bool detectConflicts, bool isImplicit) {
            ...
            ec = sccoredb.star_create_transaction_and_set_current(flags, 0, out handle, out verify);
            if (ec == 0) {
                try {
                    TransactionHandle th = new TransactionHandle(handle, verify, flags, index);
                    if (isImplicit)
        ...
        internal static uint DisposeNoException(TransactionHandle handle) {
            ...
            if (handle.IsAlive) {
                ec  = sccoredb.star_free_transaction(handle.handle, handle.verify);
                if (ec != 0)
                    return ec;
            }
            ...
            if (handle.index < ShortListCount) {
                unsafe {
                    ...
                    if (!keptHandle.HasTemporaryRef()) {
                        ec = sccoredb.star_transaction_is_dirty(handle.handle, handle.verify, &isDirty);
                        if (ec == 0)
                            ec = sccoredb.star_free_transaction(handle.handle, handle.verify);
        ...
        internal static void SetCurrentTransaction(TransactionHandle handle) {
            ...
            uint ec = sccoredb.star_set_current_transaction(0, handle.handle, handle.verify);
            if (ec == 0) {
        ...
        internal static unsafe void Cleanup() {
            ...
            for (int i = 0; i < fastCount; i++) {
                ...
                if (!th.HasTransferedOwnership()) {
                    ec = sccoredb.star_free_transaction(th.handle, th.verify);
                    if (ec == 0)
                        continue;
                    // TODO:
            ...
            if (used > ShortListCount){
                ...
                for (int i = 0; i < slowList.Count; i++) {
                    th = slowList[i];
                    if (!th.HasTransferedOwnership()) {
                        ec = ec  = sccoredb.star_free_transaction(th.handle, th.verify);
                        if (ec == 0)
                            continue;
        ...
        public void Rollback(TransactionHandle handle) {
            Scope(handle, () => {
                uint ec = sccoredb.star_rollback();
                if (ec == 0) return;
