...
START CMD /C "timeout 2 && sccode.exe %DB_NAME% --DatabaseDir=%DB_DIR% --OutputDir=%DB_OUT_DIR% --TempDir=%DB_OUT_DIR% --AutoStartExePath=s\NetworkIoTest\NetworkIoTest.exe --SchedulerCount=2 --ChunksNumber=8192 --UserArguments="DbNumber=0 PortNumber=80 TestType=MODE_STANDARD_BROWSER""
