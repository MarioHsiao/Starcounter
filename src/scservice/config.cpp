    ...
    wchar_t **default_apps_port)
{
    ...
    *pserver_database_dir = (wchar_t *)malloc(str_size_bytes);
    ...
    str_size_bytes = str_num_chars * sizeof(wchar_t);
    *default_apps_port = (wchar_t *)malloc(str_size_bytes);
    if (!*default_apps_port) goto end;

    // Converting default apps port from UTF-8 to wchar_t.
    num_chars_converted = MultiByteToWideChar(CP_UTF8, 0, default_apps_port_elem->value(), -1, *default_apps_port, (int)str_num_chars);
    if (0 == num_chars_converted) goto end;
