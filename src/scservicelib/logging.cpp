...
void LogWriteCritical(const wchar_t* msg)
{
	...
	if (msg)
	{
		err_code = star_kernel_write_to_logs(g_sc_log_handle_, SC_ENTRY_CRITICAL, 0,
...
{
	uint32_t err_code = star_kernel_write_to_logs(g_sc_log_handle_, SC_ENTRY_ERROR, 0,
		reinterpret_cast<const ucs2_char *>(msg));
...
{
	uint32_t err_code = star_kernel_write_to_logs(g_sc_log_handle_, SC_ENTRY_DEBUG, 0,
		reinterpret_cast<const ucs2_char *>(msg));
